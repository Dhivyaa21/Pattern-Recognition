...
public class MNISTReader {
    ...
    public MNISTReader(String dataFile, String labelFile) {
        load(dataFile,labelFile);
    }

    public void load(String dataFile, String labelFile) {
    ...
    private byte[][] loadData(String filename) {
        try {
            ...
            int magicNo = dis.readInt(); // should be 2051
            numOfImages = dis.readInt();
            numOfRows = dis.readInt();
            numOfCols = dis.readInt();
            numOfCells = numOfRows * numOfCols;

            byte[][] data = new byte[numOfImages][numOfCells];

            for (int i = 0; i < numOfImages; i++) {
                data[i] = new byte[numOfCells];
    ...
    private byte[] loadLabels(String filename) {
        try {
            DataInputStream dis = new DataInputStream(new FileInputStream(filename));
            int magicNo = dis.readInt(); // should be 2049
